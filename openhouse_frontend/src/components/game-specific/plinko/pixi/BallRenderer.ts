    ...
    this.balls.forEach((ball) => {
      ...
      if (ball.progress >= 1) {
        ...
        if (ball.currentRow >= this.rows) {
          ...
          // Add 20px offset to match SlotRenderer's positioning + half slot height for centering
          const y = LAYOUT.DROP_ZONE_HEIGHT + this.rows * LAYOUT.PEG_SPACING_Y + 20 + LAYOUT.SLOT_HEIGHT / 2;
